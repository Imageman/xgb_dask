...
def get_xgboost_model(space):
    ...
    model = xgboost.XGBClassifier(max_depth=_max_depth,
...
def test(model, df_test_X, df_test_Y):
            ...
            print("Accuracy: {:.2f}% F1: {:.2f}% ".format(accuracy * 100.0, f1*100))
            return f1
...
space ={
        ...
        'max_depth': hp.quniform("max_depth", 9, 11, 1),
        'n_estimators':hp.quniform("n_estimators", 195, 200, 1),
